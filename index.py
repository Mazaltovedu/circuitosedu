...
app = Flask(__name__)
# Permissão total para o site no GitHub Pages
...
def get_gemini_response(user_input):
    """Conecta ao Gemini 1.5 Flash usando a URL v1beta (mais estável)"""
    api_key = os.environ.get('GEMINI_API_KEY', '').strip().replace('"', '').replace("'", "")
    
    if not api_key:
        return "Erro: GEMINI_API_KEY não foi encontrada na Vercel."

    # URL v1beta para resolver o erro 404 das suas imagens
    ...
    try:
        ...
        if response.status_code != 200:
            # Se der erro, mostramos o motivo real do Google
            return f"Erro na IA (Google Status {response.status_code}). Verifique se a chave é válida."
